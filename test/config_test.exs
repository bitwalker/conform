defmodule ConfigTest do
  ...
  test "can load config.exs containing a single nested list" do
  ...
  test "can translate config.exs + schema + conf with single nested list to sys.config" do
    path   = Path.join(["test", "configs", "single_nested_list.exs"])
    config = path |> Mix.Config.read! |> Macro.escape
    schema = Conform.Schema.from_config(config)
    {:ok, conf} = Path.join(["test", "confs", "single_nested_list.conf"]) |> Conform.Conf.from_file
    sysconfig = Conform.Translate.to_config(schema, config, conf)
    assert [my_app: [rx_pattern: [~r/[A-Z]+/],
                     sublist: [[opt1: "val1", opt2: "val two"]]]] == sysconfig
  end
end
