[
  ...
  transforms: [
    ...
    "lager.handlers": fn conf ->
      ...
      |> IO.inspect(label: "lager.handlers")
      |> Enum.reduce([], fn
    ...
    "my_app.complex_list.*": fn conf ->
      selected = Conform.Conf.find(conf, "my_app.complex_list.$name")
      IO.inspect selected, label: "complex_list.*"
      #:ok = Conform.Conf.remove(conf, "my_app.complex_list")
      Enum.reduce(selected, [], fn
        {[_,_, name, key], val}, acc ->
          name = :"#{name}"
          key = :"#{key}"
          case get_in(acc, [name]) do
            nil ->
              acc
              |> put_in([name], [])
              |> put_in([name, key], val)
